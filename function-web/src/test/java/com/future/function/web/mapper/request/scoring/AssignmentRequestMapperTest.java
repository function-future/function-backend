...
public class AssignmentRequestMapperTest {
  ...
  public void setUp() {
    ...
    when(validator.validate(assignmentWebRequest)).thenReturn(
      assignmentWebRequest);
    when(validator.validate(copyAssignmentWebRequest)).thenReturn(
  ...
  public void testToAssignmentFromStringDataJson() {
    ...
    Assignment actual = assignmentRequestMapper.toAssignment(
      assignmentWebRequest, ASSIGNMENT_BATCH);
  ...
  public void testToAssignmentFromStringDataJsonAndStringIdSuccess() {
    ...
    Assignment actual = assignmentRequestMapper.toAssignmentWithId(
      ASSIGNMENT_ID, assignmentWebRequest, ASSIGNMENT_BATCH);
    assertThat(actual.getDescription()).isEqualTo(assignment.getDescription());
    assertThat(actual.getTitle()).isEqualTo(assignment.getTitle());
    verify(validator).validate(assignmentWebRequest);
  }
  ...
  public void testToCopyAssignmentWebRequest() {
    ...
    CopyAssignmentWebRequest request =
      assignmentRequestMapper.validateCopyAssignmentWebRequest(
        copyAssignmentWebRequest);
