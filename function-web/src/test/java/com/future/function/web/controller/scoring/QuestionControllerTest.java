...
public class QuestionControllerTest extends TestHelper {
  ...
  public void setUp() {
    ...
    questionWebResponse = QuestionWebResponse.builder()
      .label(QUESTION_TEXT)
    ...
    questionPage = new PageImpl<>(
      Collections.singletonList(question), pageable, 10);

    DATA_RESPONSE = QuestionResponseMapper.toQuestionWebResponse(this.question);

    CREATED_DATA_RESPONSE = QuestionResponseMapper.toQuestionWebResponse(
      HttpStatus.CREATED, this.question);

    PAGING_RESPONSE = QuestionResponseMapper.toQuestionPagingResponse(
      questionPage);
  ...
  public void getAllQuestionForQuestionBank() throws Exception {
    ...
    when(questionService.findAllByQuestionBankId(QUESTION_BANK_ID,
                                                 pageable
    )).thenReturn(questionPage);

    mockMvc.perform(get(
      "/api/scoring/question-banks/" + QUESTION_BANK_ID + "/questions").cookie(
      cookies)
                      .param("page", "1")
                      .param("size", "10"))
  ...
  public void getQuestionDetailById() throws Exception {
    ...
    mockMvc.perform(get(
      "/api/scoring/question-banks/" + QUESTION_BANK_ID + "/questions/" +
      QUESTION_ID).cookie(cookies))
      .andExpect(status().isOk())
  ...
  public void createQuestionForQuestionBank() throws Exception {
    ...
    when(questionService.createQuestion(question, QUESTION_BANK_ID)).thenReturn(
      question);
    when(questionRequestMapper.toQuestion(questionWebRequest)).thenReturn(
      question);
    ...
    mockMvc.perform(post(
      ...
      cookies)
                      ...
                      .content(webRequestJacksonTester.write(questionWebRequest)
                                 .getJson()))
  ...
  public void updateQuestionForQuestionBank() throws Exception {
    ...
    when(questionService.updateQuestion(question, QUESTION_BANK_ID)).thenReturn(
      question);
    when(questionRequestMapper.toQuestion(questionWebRequest,
                                          QUESTION_ID
    )).thenReturn(question);

    questionWebRequest.getOptions()
      .get(0)
      .setId(OPTION_ID);
    when(questionRequestMapper.toQuestion(questionWebRequest,
                                          QUESTION_ID
