...
public class QuizControllerTest extends TestHelper {
  ...
  public void setUp() {
    ...
    quizWebRequest = QuizWebRequest.builder()
      .title(QUIZ_TITLE)
      .description(QUIZ_DESCRIPTION)
      .startDate(QUIZ_DATE)
      .endDate(QUIZ_DATE)
    ...
    copyQuizWebRequest = CopyQuizWebRequest.builder()
      .quizId(QUIZ_ID)
    ...
    DATA_RESPONSE = QuizResponseMapper.toQuizWebDataResponse(this.quiz);

    CREATED_DATA_RESPONSE = QuizResponseMapper.toQuizWebDataResponse(
      HttpStatus.CREATED, this.quiz);

    PAGING_RESPONSE = QuizResponseMapper.toQuizWebPagingResponse(quizPage);
    ...
    when(requestMapper.toQuiz(quizWebRequest, QUIZ_BATCH_CODE)).thenReturn(
      quiz);
    when(requestMapper.toQuiz(QUIZ_ID, quizWebRequest,
                              QUIZ_BATCH_CODE
    )).thenReturn(quiz);
    when(
      requestMapper.validateCopyQuizWebRequest(copyQuizWebRequest)).thenReturn(
      copyQuizWebRequest);
  }
  ...
  public void testFindQuizById() throws Exception {
    ...
    mockMvc.perform(get(
      "/api/scoring/batches/" + QUIZ_BATCH_CODE + "/quizzes/" + QUIZ_ID).cookie(
      cookies))
  ...
  public void testFindAllQuizByPagingParameters() throws Exception {
    ...
    mockMvc.perform(get(
      "/api/scoring/batches/" + QUIZ_BATCH_CODE + "/quizzes").cookie(cookies)
                      .param("page", "1")
                      .param("size", "10"))
      .andExpect(status().isOk())
  ...
  public void testFindAllQuizWithoutPagingParameters() throws Exception {
    ...
    mockMvc.perform(
      get("/api/scoring/batches/" + QUIZ_BATCH_CODE + "/quizzes").cookie(
        cookies))
      .andExpect(status().isOk())
      .andExpect(content().json(
        pagingResponseJacksonTester.write(PAGING_RESPONSE)
  ...
  public void testCreateQuizPassQuizWebRequest() throws Exception {
    ...
    mockMvc.perform(post(
      "/api/scoring/batches/" + QUIZ_BATCH_CODE + "/quizzes").cookie(cookies)
                      ...
                      .content(webRequestJacksonTester.write(quizWebRequest)
                                 .getJson()))
  ...
  public void testCopyQuizWithTargetBatch() throws Exception {
    ...
    mockMvc.perform(post(
      "/api/scoring/batches/" + QUIZ_BATCH_CODE + "/quizzes/copy").cookie(
      cookies)
                      .contentType(MediaType.APPLICATION_JSON_VALUE)
