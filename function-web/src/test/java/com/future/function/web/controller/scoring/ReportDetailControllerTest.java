  ...
  public void setUp() {
    ...
    CREATED_DATA_RESPONSE = ResponseHelper.toDataResponse(HttpStatus.CREATED,
                                                          Collections.singletonList(
                                                            createdReportDetailWebResponse)
    );

    when(fileProperties.getUrlPrefix()).thenReturn(URL_PREFIX);
    when(requestMapper.toReportDetailList(reportDetailScoreWebRequest,
                                          REPORT_ID
    ...
    mockMvc.perform(post(
      ...
      .andExpect(content().json(
        dataResponseJacksonTester.write(CREATED_DATA_RESPONSE)
          .getJson()));
    verify(requestMapper).toReportDetailList(
      reportDetailScoreWebRequest, REPORT_ID);
    verify(reportService).giveScoreToReportStudents(
