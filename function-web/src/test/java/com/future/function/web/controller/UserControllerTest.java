...
public class UserControllerTest {
  ...
  @MockBean private UserRequestMapper userRequestMapper;
  ...
  public void testGivenCallToUsersApiByGettingUsersFromUserServiceReturnPagingResponseOfUsers()
    ...
    given(userService.getUsers(Role.STUDENT, PAGEABLE))
        .willReturn(new PageImpl<>(studentsList, PAGEABLE, studentsList.size()));

    MockHttpServletResponse response =
        mockMvc
            .perform(get("/api/core/users").param("role", Role.STUDENT.name()))
            .andReturn()
            .getResponse();
  ...
  public void testGivenEmailFromPathVariableByDeletingUserByEmailReturnBaseResponseOK()
    ...
    MockHttpServletResponse response =
        mockMvc.perform(delete("/api/core/users/" + STUDENT_EMAIL)).andReturn().getResponse();
  ...
  public void testGivenEmailFromPathVariableByGettingUserByEmailReturnDataResponseUser()
    ...
    MockHttpServletResponse response =
        mockMvc.perform(get("/api/core/users/" + STUDENT_EMAIL)).andReturn().getResponse();
