...
public class QuestionnaireParticipantRepositoryTest {
  ...
  public void SetUp() {
    ...
    QuestionnaireParticipant questionnaireParticipant2 =
      QuestionnaireParticipant.builder()
        .id(ID_2)
  ...
  @Test
  public void testByGivenMemberAndParticipantTypeByFindingAllQuestionnaireParticipantReturnPagedQuestionnaireParticipant() {
    ...
    Page<QuestionnaireParticipant> questionnaireParticipants =
      questionnaireParticipantRepository.findAllByMemberAndParticipantTypeAndDeletedFalseOrderByCreatedAtDesc(
        user1, ParticipantType.fromString("APPRAISER"), PAGEABLE);

    assertThat(questionnaireParticipants.getTotalElements()).isEqualTo(1);
    assertThat(questionnaireParticipants.getContent()
                 .get(0)
    ...
    assertThat(questionnaireParticipants.getContent()
                 .get(0)
                 .getQuestionnaire()
                 .getId()).isEqualTo(ID_QUESTIONNAIRE1);
    //      assertThat(questionnaireParticipants.getContent().get(1).getId())
    //      .isEqualTo(ID_2);
    //      assertThat(questionnaireParticipants.getContent().get(1)
    //      .getMember().getId()).isEqualTo(ID_USER_1);
    //      assertThat(questionnaireParticipants.getContent().get(1)
    //      .getQuestionnaire().getId()).isEqualTo(ID_QUESTIONNAIRE2);
  }
