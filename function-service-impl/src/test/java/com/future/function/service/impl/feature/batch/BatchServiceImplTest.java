...
public class BatchServiceImplTest {
  ...
  public void setUp() {
    ...
    batch = Batch.builder().number(FIRST_BATCH_NUMBER).deleted(false).build();

    when(batchRepository.findByNumberAndDeletedIsFalse(FIRST_BATCH_NUMBER))
  ...
  @Test
  public void
      testGivenMethodCallToCreateBatchWhileNoBatchExistedBeforeByCreatingBatchReturnNewBatchObject() {

    when(batchRepository.findFirstByIdIsNotNullOrderByUpdatedAtDesc()).thenReturn(Optional.empty());
    ...
    verify(batchRepository, times(1)).findFirstByIdIsNotNullOrderByUpdatedAtDesc();
    verify(batchRepository, times(1)).save(batch);
  }

  @Test
  public void
      testGivenMethodCallToCreateBatchWhileAUndeletedBatchExistedBeforeByCreatingBatchReturnNewBatchObject() {

    when(batchRepository.findFirstByIdIsNotNullOrderByUpdatedAtDesc())
        .thenReturn(Optional.of(batch));

    Batch secondBatch = Batch.builder().number(SECOND_BATCH_NUMBER).deleted(false).build();
  ...
  @Test
  public void
      testGivenMethodCallToCreateBatchWhileADeletedBatchExistedBeforeByCreatingBatchReturnNewBatchObject() {
  ...
  public void testGivenExistingBatchInDatabaseByFindingBatchByNumberReturnBatchObject() {
    ...
    verify(batchRepository, times(1)).findByNumberAndDeletedIsFalse(FIRST_BATCH_NUMBER);
  }
  ...
  public void testGivenNonExistingBatchInDatabaseByDeletingBatchByNumberReturnRuntimeException() {
    ...
    when(batchRepository.findByNumberAndDeletedIsFalse(SECOND_BATCH_NUMBER)).thenReturn(Optional.empty());
    ...
    verify(batchRepository, times(1)).findByNumberAndDeletedIsFalse(SECOND_BATCH_NUMBER);
  }

  @Test
  public void testGivenExistingBatchInDatabaseByDeletingBatchByNumberReturnSuccessfulDeletion() {

    when(batchRepository.findByNumberAndDeletedIsFalse(FIRST_BATCH_NUMBER)).thenReturn(Optional.of(batch));
