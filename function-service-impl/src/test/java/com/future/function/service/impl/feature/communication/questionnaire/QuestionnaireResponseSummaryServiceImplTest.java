...
public class QuestionnaireResponseSummaryServiceImplTest {
  ...
  @InjectMocks
  private QuestionnaireResponseSummaryServiceImpl
  ...
  public void setUp() {
    ...
    questionnaireResponseSummary1 = QuestionnaireResponseSummary.builder()
  ...
  public void tearDown() {
    ...
    verifyNoMoreInteractions(questionnaireResponseSummaryRepository,
  ...
  public void getQuestionnairesSummariesBasedOnAppraisee() {
    ...
    when(
      questionnaireResponseSummaryRepository.findAllByAppraiseeAndDeletedFalse(
        user1, PAGEABLE)).thenReturn(
      new PageImpl<>(Collections.singletonList(questionnaireResponseSummary1),
                     PAGEABLE, 1));

    Page<QuestionnaireResponseSummary> questionnaireResonseSummaryPage =
      questionnaireResponseSummarySevice.getQuestionnairesSummariesBasedOnAppraisee(
        user1, PAGEABLE);

    assertThat(questionnaireResonseSummaryPage.getTotalElements()).isEqualTo(1);
    assertThat(questionnaireResonseSummaryPage.getContent()
                 .get(0)
                 .getId()).isEqualTo(QUESTIONNAIRE_RESPONSE_SUMMARY_ID_1);
  ...
  public void getQuestionnaireResponseSummaryById() {
    ...
    when(questionnaireResponseSummaryRepository.findOne(
      QUESTIONNAIRE_RESPONSE_SUMMARY_ID_1)).thenReturn(
      questionnaireResponseSummary1);

    QuestionnaireResponseSummary questionnaireResponseSummaryResult =
      questionnaireResponseSummarySevice.getQuestionnaireResponseSummaryById(
        QUESTIONNAIRE_RESPONSE_SUMMARY_ID_1);

    assertThat(questionnaireResponseSummaryResult.getId()).isEqualTo(
      QUESTIONNAIRE_RESPONSE_SUMMARY_ID_1);

    verify(questionnaireResponseSummaryRepository).findOne(
      QUESTIONNAIRE_RESPONSE_SUMMARY_ID_1);
  }
  ...
  public void getQuestionsDetailsFromQuestionnaireResponseSummaryIdAndAppraisee() {
    ...
    when(questionnaireResponseSummaryRepository.findOne(
      ...
      questionnaireResponseSummary1);
    when(
      questionResponseSummaryRepository.findAllByQuestionnaireAndAppraiseeAndDeletedFalse(
        questionnaire1, user1)).thenReturn(
      Arrays.asList(questionResponseSummary1));

    List<QuestionResponseSummary> questionResponseSummaries =
      questionnaireResponseSummarySevice.getQuestionsDetailsFromQuestionnaireResponseSummaryIdAndAppraisee(
        QUESTIONNAIRE_RESPONSE_SUMMARY_ID_1, user1);

    assertThat(questionResponseSummaries.size()).isEqualTo(1);
    assertThat(questionResponseSummaries.get(0)
                 .getId()).isEqualTo(QUESTION_RESPONSE_SUMMARY_ID_1);

    verify(questionnaireResponseSummaryRepository).findOne(
      QUESTIONNAIRE_RESPONSE_SUMMARY_ID_1);
    verify(
      questionResponseSummaryRepository).findAllByQuestionnaireAndAppraiseeAndDeletedFalse(
      questionnaire1, user1);
  }

  @Test
  public void getQuestionResponseSummaryById() {

    when(questionResponseSummaryRepository.findOne(
      QUESTION_RESPONSE_SUMMARY_ID_1)).thenReturn(questionResponseSummary1);

    QuestionResponseSummary questionResponseSummaryResult =
      questionnaireResponseSummarySevice.getQuestionResponseSummaryById(
        QUESTION_RESPONSE_SUMMARY_ID_1);

    assertThat(questionResponseSummaryResult.getId()).isEqualTo(
      QUESTION_RESPONSE_SUMMARY_ID_1);

    verify(questionResponseSummaryRepository).findOne(
      QUESTION_RESPONSE_SUMMARY_ID_1);
  ...
  public void getQuestionResponseByQuestionResponseSummaryId() {
    ...
    when(
      questionResponseRepository.findAllByQuestionQuestionnaireAndAppraiseeAndDeletedFalse(
        question1, user1)).thenReturn(Arrays.asList(questionResponse1));
    ...
    List<QuestionResponse> questionResponses =
      questionnaireResponseSummarySevice.getQuestionResponseByQuestionResponseSummaryId(
        QUESTION_RESPONSE_SUMMARY_ID_1);

    assertThat(questionResponses.size()).isEqualTo(1);
    assertThat(questionResponses.get(0)
                 .getId()).isEqualTo(QUESTION_RESPONSE_ID_1);
