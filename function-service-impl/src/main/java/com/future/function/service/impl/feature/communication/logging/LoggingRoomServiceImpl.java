...
public class LoggingRoomServiceImpl implements LoggingRoomService {
  ...
  @Autowired
  public LoggingRoomServiceImpl(
    LoggingRoomRepository loggingRoomRepository, UserService userService
  ) {
  ...
  @Override
  public Page<LoggingRoom> getLoggingRoomsByMember(
    ...
    return loggingRoomRepository.findAllByDeletedFalseOrderByCreatedAtDesc(
      pageable);
  }

  @Override
  public Page<LoggingRoom> getLoggingRoomsByMemberWithKeyword(
    String keyword, String memberId, Pageable pageable
  ) {

    if (userService.getUser(memberId)
      .getRole()
      .equals(Role.STUDENT)) {
      return loggingRoomRepository.findAllByTitleContainingIgnoreCaseAndMembersAndDeletedFalseOrderByCreatedAtDesc(
        keyword, userService.getUser(memberId), pageable);
    }
    return loggingRoomRepository.findAllByTitleContainingIgnoreCaseAndDeletedFalseOrderByCreatedAtDesc(
      keyword, pageable);
  }
  ...
  private LoggingRoom copyProperties(
    LoggingRoom savedRoom, LoggingRoom loggingRoom
  ) {
  ...
  private LoggingRoom updateMember(
    LoggingRoom savedLoggingRoom, LoggingRoom newLoggingRoom
  ) {
  ...
  private LoggingRoom setMembers(LoggingRoom loggingRoom) {
    ...
    if (loggingRoom.getMembers() != null) {
      loggingRoom.getMembers()
        .forEach(member -> members.add(userService.getUser(member.getId())));
    }
