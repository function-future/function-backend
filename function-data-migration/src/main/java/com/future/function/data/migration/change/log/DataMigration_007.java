...
public class DataMigration_007 {
  ...
  public void questionBankMigration(MongoTemplate mongoTemplate) {
    ...
    questionBank.append(
      FieldName.QuestionBank.DESCRIPTION, QUESTION_BANK_DESCRIPTION);
    questionBank.append(
  ...
  @ChangeSet(author = "oliver",
             order = "0002",
             id = "questionMigration")
  public void questionMigration(MongoTemplate mongoTemplate) {

    BasicDBObject question = new BasicDBObject();
    String questionBankId = mongoTemplate.findAll(QuestionBank.class)
      .get(0)
      .getId();
    question.append(FieldName.Question.QUESTION_BANK,
                    new DBRef(DocumentName.QUESTION_BANK,
  ...
  public void optionMigration(MongoTemplate mongoTemplate) {
    ...
    String questionId = mongoTemplate.findAll(Question.class)
      .get(0)
      .getId();
    for (int i = 0; i < 4; i++) {
      BasicDBObject option = new BasicDBObject();
      option.append(FieldName.Option.QUESTION,
                    new DBRef(DocumentName.QUESTION, new ObjectId(questionId))
      );
      option.append(FieldName.Option.LABEL,
                    OPTION_lABEL_PREFIX.concat(String.valueOf(i + 1))
      );
      option.append(
        FieldName.BaseEntity.CREATED_AT, System.currentTimeMillis());
