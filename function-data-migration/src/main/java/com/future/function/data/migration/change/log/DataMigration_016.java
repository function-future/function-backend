...
public class DataMigration_016 {
  ...
  public void addStudentQuizIndexes(MongoDatabase mongoDatabase) {
    ...
    mongoDatabase.getCollection(DocumentName.STUDENT_QUIZ)
      .createIndex(
        Indexes.ascending(
          STUDENT_QUIZ_USER_ID_AND_QUIZ_ID_AND_DELETED.getFields()),
        new IndexOptions().name(
          STUDENT_QUIZ_USER_ID_AND_QUIZ_ID_AND_DELETED.name())
      );
  ...
  public void addStudentQuestionIndexes(MongoDatabase mongoDatabase) {
    ...
    mongoDatabase.getCollection(DocumentName.STUDENT_QUESTION)
      .createIndex(
        Indexes.ascending(
          STUDENT_QUESTION_STUDENT_QUIZ_DETAIL_ID_AND_NUMBER.getFields()),
        new IndexOptions().name(
          STUDENT_QUESTION_STUDENT_QUIZ_DETAIL_ID_AND_NUMBER.name())
      );
  }
  ...
  public void addStudentQuizDetailIndexes(MongoDatabase mongoDatabase) {
    ...
    mongoDatabase.getCollection(DocumentName.STUDENT_QUIZ_DETAIL)
      .createIndex(
        Indexes.descending(
          STUDENT_QUIZ_DETAIL_STUDENT_QUIZ_ID_AND_CREATED_AT_AND_DELETED.getFields()),
        new IndexOptions().name(
          STUDENT_QUIZ_DETAIL_STUDENT_QUIZ_ID_AND_CREATED_AT_AND_DELETED.name())
      );
  }
